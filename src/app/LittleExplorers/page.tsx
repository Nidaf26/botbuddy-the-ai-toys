...
function Page() {
  ...
  const [Data, setData] = useState<LittleExplorer[]>([]);
  useEffect(() => {
    const fetchdata = async () => {
      setloading(true);
      try {
        const littleExplorer: LittleExplorer[] = await client.fetch(`
          *[_type == "LittleExplorers"]{
            _id,
            productname,
            productdescription,
            productPrice,
        ...
        console.log(littleExplorer);
        setData(littleExplorer);
      } catch (error) {
        console.log(error);
  ...
  return (
    <section className="grid grid-cols-2 grid-rows-1 bg-[#dfd9f8] gap-2 p-11 msm:grid-cols-1 msm:grid-rows-2 xsm:grid-cols-1 xsm:grid-rows-2 ssm:grid-cols-1 ssm:grid-rows-2 ">
      {Data.map((product) => (
        <div className="flex justify-center items-center" key={product._id}>
          ...
          >
            ...
            <span className="bg-white flex justify-center flex-col items-center gap-2">
              ...
              <Link
                href={`/LittleExplorers/${product._id}`}
